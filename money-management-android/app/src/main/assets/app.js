...
const DEFAULT_CATEGORIES={
  expense:[['Food','খাবার'],['Transport','যাতায়াত'],['Shopping','কেনাকাটা'],['Bills','বিল'],['Health','চিকিৎসা'],['Other Expense','অন্যান্য']],
  income:[['Salary','বেতন'],['Business','ব্যবসা'],['Gift','উপহার'],['Refund','ফেরত'],['Other Income','অন্যান্য']]
};

function ensureCategoriesForProfile(profileId){
  const has=state.categories.some(c=>c.profileId===profileId);
  if(has)return;
  Object.entries(DEFAULT_CATEGORIES).forEach(([type,items])=>items.forEach(([name,subcategory])=>state.categories.push({id:uid(),profileId,type,name,subcategory,createdAt:Date.now()})));
}
...
function categoryOptions(type,selectedId,legacyName){
  ...
  let options='<option value="">Category নির্বাচন (ঐচ্ছিক)</option>';
  options+=list.map(c=>`<option value="${c.id}" ${c.id===selectedId?'selected':''}>${esc(categoryLabel(c))}</option>`).join('');
  if(selectedId&&!list.some(c=>c.id===selectedId)&&legacyName)options+=`<option value="" selected>${esc(legacyName)} (পুরোনো)</option>`;
  return options;
}
function renderCategorySelects(){const ex=$('expenseCategory').value,inc=$('incomeCategory').value;$('expenseCategory').innerHTML=categoryOptions('expense',ex);$('incomeCategory').innerHTML=categoryOptions('income',inc)}
function selectedCategoryMeta(type,selectId){const c=getCategory($(selectId).value);if(!c||c.type!==type)return{categoryId:'',categoryName:'Uncategorized'};return{categoryId:c.id,categoryName:categoryLabel(c)}}

function clearRow(type){['Amount','Subject','Remarks'].forEach(x=>$(type+x).value='');$(type+'Date').value=today();$(type+'Category').value=''}
function cancelAllQuickInputs(){clearRow('expense');clearRow('income');toast('Save হওয়ার আগের input বাতিল হয়েছে')}

function createTx(type,a,subject,remarks,date,accountOverride,meta){
  const t={id:uid(),profileId:state.active,type,amount:+a,subject:(subject||'').trim(),remarks:(remarks||'').trim(),date:date||today(),time:nowTime(),account:accountOverride||account,categoryId:'',categoryName:'Uncategorized',createdAt:Date.now(),updatedAt:Date.now(),...(meta||{})};
  state.tx.unshift(t);return t;
}
function createAndSaveTx(type,a,subject,remarks,date,accountOverride,meta){createTx(type,a,subject,remarks,date,accountOverride,meta);save();render();toast(type==='income'?'আয় যোগ হয়েছে':'ব্যয় যোগ হয়েছে')}
...
function scheduleSave(type){setTimeout(()=>{const active=document.activeElement;if(active?.dataset?.row===type)return;quickSave(type)},170)}
...
function renderReports(){const f=$('reportFrom').value,t=$('reportTo').value,list=activeTx().filter(x=>(!f||x.date>=f)&&(!t||x.date<=t));$('reportSummary').innerHTML=summaryHtml(summary(list));const groups={};list.forEach(x=>{const k=x.categoryName||'Uncategorized';groups[k]=(groups[k]||0)+(x.type==='expense'?-x.amount:x.amount)});$('categoryBreakdown').innerHTML=Object.entries(groups).sort((a,b)=>Math.abs(b[1])-Math.abs(a[1])).map(([k,v])=>`<div class="category-line"><span>${esc(k)}</span><strong class="${v>=0?'money-in':'money-out'}">${v>=0?'+':''}${money(v)}</strong></div>`).join('')||'<div class="empty">Data নেই</div>'}
...
function resetCategoryForm(){categoryEditId=null;$('categoryType').value='expense';$('categoryName').value='';$('categorySubcategory').value='';$('saveCategory').textContent='Category Add';$('cancelCategoryEdit').hidden=true}
function renderCategoryManager(){const list=[...activeCategories()].sort((a,b)=>a.type.localeCompare(b.type)||a.name.localeCompare(b.name));$('categoryManageList').innerHTML=list.map(c=>`<div class="category-manage-row"><span class="category-type-badge ${c.type}">${c.type==='income'?'Income':'Expense'}</span><div class="category-name"><strong>${esc(c.name)}</strong><small>${esc(c.subcategory||'Subcategory নেই')}</small></div><div class="category-row-actions"><button class="category-edit" data-category-edit="${c.id}">Edit</button><button class="category-delete" data-category-delete="${c.id}">Delete</button></div></div>`).join('')||'<div class="empty">Category নেই</div>'}
function saveCategory(){const type=$('categoryType').value,name=$('categoryName').value.trim(),subcategory=$('categorySubcategory').value.trim();if(!name){toast('Category নাম দিন');return}const duplicate=activeCategories(type).find(c=>c.id!==categoryEditId&&c.name.toLowerCase()===name.toLowerCase()&&(c.subcategory||'').toLowerCase()===subcategory.toLowerCase());if(duplicate){toast('এই Category আগে থেকেই আছে');return}if(categoryEditId){const c=getCategory(categoryEditId);if(c)Object.assign(c,{type,name,subcategory,updatedAt:Date.now()});toast('Category updated')}else{state.categories.push({id:uid(),profileId:state.active,type,name,subcategory,createdAt:Date.now()});toast('Category added')}resetCategoryForm();save();render()}
function editCategory(id){const c=getCategory(id);if(!c)return;categoryEditId=id;$('categoryType').value=c.type;$('categoryName').value=c.name;$('categorySubcategory').value=c.subcategory||'';$('saveCategory').textContent='Update Category';$('cancelCategoryEdit').hidden=false;$('categoryName').focus()}
...
function render(){ensureCategoriesForProfile(state.active);profiles();renderCategorySelects();renderDrafts();renderHistory();renderReports();renderReminders();renderLoans();renderCategoryManager();$('nativeBackupStatus').textContent=window.MoneyBackup?'Android/Drive backup প্রস্তুত':'Local backup চালু'}
...
document.querySelectorAll('.bottom-nav button').forEach(b=>b.onclick=()=>{const x=b.dataset.nav;if(x==='quick'){page('quick');$('quickSection').scrollIntoView()}else if(x==='history'){page('history');$('historySection').scrollIntoView()}else page(x)});
$('expenseDate').value=today();$('incomeDate').value=today();$('reminderDate').value=today();$('loanStartDate').value=today();$('paymentDate').value=today();resetLoanForm();resetCategoryForm();render();
})();